...
import numpy as np
from algoritmes.utils import compute_color_for_labels
from algoritmes.utils import get_names
from collections import deque

names = get_names()
data_deque = {}
#число пикселей отображения пути
N = 120

def draw_ui_box(x, img, label=None, color=None, line_thickness=None):
    # Отображает одну ограничивающую рамку на изображении img
    tl = line_thickness or round(
        0.002 * (img.shape[0] + img.shape[1]) / 2) + 1  # толщина строки/шрифта
...
def draw_border(img, pt1, pt2, color, thickness, r, d):
    x1, y1 = pt1
    x2, y2 = pt2
    # Вверху слева из коллекций импортируйте размер (x1, y1 + r + d), цвет, толщину)
...
def draw_boxes(img, bbox_xyxy, class_ids, identities=None, draw_trails=False, offset=(0, 0), class_names=None):
    ...
    # удалить отслеживаемую точку из буфера, если объект потерян
    if draw_trails:
    ...
    for i, box in enumerate(bbox_xyxy):
        ...
        # Рисуем контуры
        # код для нахождения центра нижнего края
        center = (int((x2+x1) / 2), int((y2+y2)/2))

        if draw_trails:
            # создать новый буфер для нового объекта
            if id not in data_deque:  
                data_deque[id] = deque(maxlen= N)
...
def drawtrails(data_deque, id, color, img):
    ...
    for i in range(1, len(data_deque[id])):
        # проверьте, равно ли значение on buffer none
        if data_deque[id][i - 1] is None or data_deque[id][i] is None:
            continue

        # генерировать динамическую толщину трасс
        thickness = int(np.sqrt(N / float(i + i)) * 1.5)
